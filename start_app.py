...
def clean_next_cache(frontend_dir):
    ...
    if os.path.exists(next_cache):
        try:
            ...
            print("[INFO] Cleared .next cache to prevent Windows EINVAL readlink errors.")
        except Exception:
            pass
...
def main():
    ...
    print("\n[2/2] Starting Next.js 14 Frontend Dev Server on http://localhost:3000...")
    frontend_cmd = ["npm.cmd" if os.name == "nt" else "npm", "run", "dev"]
    frontend_proc = subprocess.Popen(frontend_cmd, cwd=frontend_dir)

    print("\n" + "=" * 65)
    print("SUCCESS: Full-Stack App Starting!")
